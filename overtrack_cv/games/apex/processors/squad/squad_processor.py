...
class SquadProcessor(Processor):
    ...
    def process(self, frame: Frame):
        ...
        frame.apex.squad_match = round(float(np.max(frame.apex.squad.champion)), 5)
        if frame.apex.squad_match > 0.9:
            self.REGIONS.draw(frame.debug_image)

        _draw_squad(frame.debug_image, frame.apex.squad)
        return np.max(frame.apex.squad.champion) > 0.99
